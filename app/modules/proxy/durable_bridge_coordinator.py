...
class DurableBridgeSessionCoordinator:
    ...
    async def register_turn_state(
        ...
        *,
        ...
        async with self._session() as session:
            repository = DurableBridgeRepository(session)
            snapshot = await repository.renew_session(
                session_id=session_id,
                instance_id=instance_id,
                owner_epoch=owner_epoch,
                lease_ttl_seconds=lease_ttl_seconds,
...
def _snapshot_matches_owner(
    snapshot: DurableBridgeSessionSnapshot | None,
    *,
    instance_id: str,
    owner_epoch: int,
) -> bool:
    if snapshot is None:
        return False
    return snapshot.owner_instance_id == instance_id and snapshot.owner_epoch == owner_epoch
