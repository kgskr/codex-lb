...
Trusted-header mode lets a reverse proxy own primary dashboard authentication while codex-lb still protects direct access. A request is proxy-authenticated only when the socket source is inside the configured trusted proxy CIDRs, `firewall_trust_proxy_headers=true`, and the configured trusted identity header is non-empty.

Password login remains available as a fallback when a password is configured. Without a trusted proxy identity or fallback password, dashboard APIs fail closed with `proxy_auth_required`. `disabled` mode is separate: it bypasses dashboard auth and disables password management endpoints.

Example: an operator deploys an external auth proxy that injects `X-Auth-Request-User`. The proxy IP range is listed in `firewall_trusted_proxy_cidrs`, trusted proxy headers are enabled, and the dashboard session endpoint reports `authMode: "trusted_header"` with `authenticated: true` for proxied users.
