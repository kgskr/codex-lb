...
@pytest.mark.asyncio
async def test_durable_bridge_old_owner_cannot_attach_alias_after_takeover(
    coordinator: DurableBridgeSessionCoordinator,
) -> None:
    claimed = await coordinator.claim_live_session(
        session_key_kind="session_header",
        session_key_value="sid-stale-owner",
        api_key_id=None,
        instance_id="instance-a",
        lease_ttl_seconds=60.0,
        account_id="acc-1",
    ...
    assert current.account_id == "acc-2"
