...
def _helm_template_prod(*args: str, secret_store_ref_name: str = "test-store") -> str:
    return _helm_template(
        "-f",
        str(_CHART_DIR / "values-prod.yaml"),
        "--set",
        f"externalSecrets.secretStoreRef.name={secret_store_ref_name}",
        *args,
    )
...
def test_external_secrets_install_requires_secret_store_ref_name() -> None:
    error = _helm_template_error(
        "-f",
        str(_CHART_DIR / "values-prod.yaml"),
    )

    assert "externalSecrets.secretStoreRef.name is required when externalSecrets.enabled=true" in error
