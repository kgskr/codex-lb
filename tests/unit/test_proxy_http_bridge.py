...
@pytest.mark.asyncio
async def test_stream_via_http_bridge_fails_closed_when_durable_lookup_fails_for_hard_session(
    monkeypatch: pytest.MonkeyPatch,
    ...
    monkeypatch.setattr(
        service._durable_bridge,
        "lookup_request_targets",
        AsyncMock(side_effect=RuntimeError("durable lookup unavailable")),
    )
    ...
    with pytest.raises(ProxyResponseError) as exc_info:
        async for _chunk in service._stream_via_http_bridge(
            payload,
            headers={"x-codex-session-id": "sid-hard-lookup-failure"},
            codex_session_affinity=True,
            propagate_http_errors=False,
            openai_cache_affinity=False,
            api_key=None,
    ...
    assert "continuity_fail_closed surface=http_bridge reason=durable_lookup_failed" in caplog.text
