...
class _ProxySettings:
    ...
    proxy_token_refresh_limit: int = 32
    proxy_upstream_websocket_connect_limit: int = 64
    proxy_response_create_limit: int = 64
    proxy_compact_response_create_limit: int = 16
...
async def test_connect_proxy_websocket_surfaces_local_connect_overload_without_penalizing_account(monkeypatch):
    ...
    monkeypatch.setattr(
        service,
        "_provider_adapter",
        lambda _provider_kind: SimpleNamespace(open_responses_websocket=connect_upstream),
    )
